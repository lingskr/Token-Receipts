...
const REPO_URL = "https://github.com/lingskr/Token-Receipts";
...
class EscPosBuilder {
  ...
  /**
   * Print a Codex-style cloud terminal icon in plain ASCII.
   */
  logo(): this {
    this.align("center");
    this.line(" +--------+");
...
export class ThermalPrinterRenderer {
  ...
  private buildReceipt(data: ReceiptData, shareUrl?: string): Buffer {
    ...
    if (
      ...
      for (const model of data.sessionData.modelBreakdowns) {
        ...
        b.leftRight(
          ...
          this.formatCost(model.cost, data),
        );
        b.bold(false);
        b.drawLine("-");
    ...
    // --- Total ---
    b.bold(true);
    b.leftRight("TOTAL", this.formatCost(data.sessionData.totalCost, data));
    b.bold(false);
    b.drawLine();
    b.line();
    ...
    if (data.sessionData.costStatus === "unavailable") {
      b.line("COST: unavailable");
    } else if (data.sessionData.costStatus === "estimated_api") {
      b.line("COST: estimated (API pricing)");
    }
    b.line();
    b.align("center");
  ...
  private async sendViaCups(
    ...
    const tmpFile = join(tmpdir(), `token-receipt-${Date.now()}.bin`);
  ...
  private formatCost(amount: number, data: ReceiptData): string {
    if (data.sessionData.costStatus === "unavailable") {
      return "N/A";
    }
